...
async def qspi_write_byte(dut, byte_val):
    ...
    dut.ui_in.value = dut.ui_in.value.integer | 0x01  # sclk = 1
    await ClockCycles(dut.clk, 2)
    dut.ui_in.value = dut.ui_in.value.integer & ~0x01  # sclk = 0
    await ClockCycles(dut.clk, 2)
...
async def qspi_read_byte(dut):
    ...
    dut.ui_in.value = dut.ui_in.value.integer | 0x01  # sclk = 1
    await ClockCycles(dut.clk, 2)
    hi = dut.uio_out.value.integer & 0x0F
    await ClockCycles(dut.clk, 2)
    dut.ui_in.value = dut.ui_in.value.integer & ~0x01  # sclk = 0
    ...
    dut.ui_in.value = dut.ui_in.value.integer | 0x01  # sclk = 1
    await ClockCycles(dut.clk, 2)
    lo = dut.uio_out.value.integer & 0x0F
    await ClockCycles(dut.clk, 2)
    dut.ui_in.value = dut.ui_in.value.integer & ~0x01  # sclk = 0
...
async def qspi_send_command(dut, opcode, payload=b""):
    ...
    # CS# = 0 (ui_in[1] = 0)
    dut.ui_in.value = dut.ui_in.value.integer & ~0x02
    await ClockCycles(dut.clk, 4)
...
async def test_status(dut):
    ...
    assert (dut.uio_oe.value.integer & 0x0F) == 0, "uio_oe[3:0] must be 0 while idle"

    # Query status via OP_STATUS
    dut.ui_in.value = dut.ui_in.value.integer & ~0x02  # CS# = 0
    await ClockCycles(dut.clk, 4)
    ...
    assert (dut.uio_oe.value.integer & 0x0F) == 0x0F, "uio_oe must assert during readout"
    ...
    dut.ui_in.value = dut.ui_in.value.integer | 0x02  # CS# = 1
    await ClockCycles(dut.clk, 5)

    assert (dut.uio_oe.value.integer & 0x0F) == 0, "uio_oe must release immediately on CS# high"
...
async def test_x25519(dut):
    ...
    timeout = 5000
    cycles = 0
    while (dut.uo_out.value.integer & 0x01) != 0 and cycles < timeout:
        await ClockCycles(dut.clk, 10)
        cycles += 10

    assert (dut.uo_out.value.integer & 0x01) == 0, "IRQ# did not assert after X25519 calculation"
    ...
    await ClockCycles(dut.clk, 10)
    assert (dut.uo_out.value.integer & 0x01) == 1, "irq_n did not return high after OP_IRQ_CLEAR"
...
async def test_token_seal_and_open(dut):
    ...
    # Wait for IRQ
    cycles = 0
    while (dut.uo_out.value.integer & 0x01) != 0 and cycles < 4000:
        await ClockCycles(dut.clk, 10)
        cycles += 10
    assert (dut.uo_out.value.integer & 0x01) == 0, "IRQ# did not assert after Token Seal"
    ...
    while (dut.uo_out.value.integer & 0x01) != 0 and cycles < 4000:
        await ClockCycles(dut.clk, 10)
        cycles += 10
    assert (dut.uo_out.value.integer & 0x01) == 0, "IRQ# did not assert after Token Open"
